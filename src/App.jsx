...
const LINKS = {
  ...
  compute: "https://0g.ai",
  github: "https://github.com/DingiDinigi/wraith",
};
...
function Hero({ onEnter }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", background: C.bg, overflow: "hidden", padding: "20px" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "0 32px" }}>
        ...
        <h1 style={{ fontSize: "clamp(32px, 8vw, 72px)", fontWeight: 800, lineHeight: 1.05, letterSpacing: -2, color: "#fff", marginBottom: 12, maxWidth: 700, textAlign: "center" }}>
          Your silence<br />is your <span style={{ color: C.purple }}>reputation.</span>
        </h1>

        <p style={{ fontSize: "clamp(13px, 2vw, 16px)", color: C.text2, maxWidth: 520, lineHeight: 1.7, marginBottom: 32, textAlign: "center" }}>
        ...
        <div style={{ width: "clamp(180px, 40vw, 260px)", height: "clamp(180px, 40vw, 260px)", position: "relative", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 48 }}>
          {[130, 97, 65].map((r, i) => (
...
function Dashboard({ toast }) {
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "min(300px, 100%) 1fr", gap: 20, marginBottom: 20 }}>
        <Card>
  ...
  return (
    <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(16px)", transition: "all 0.4s ease" }}>
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <div className="proof-table-head" style={{ display: "grid", gridTemplateColumns: "18px 180px 1fr 100px 90px", gap: 12, padding: "10px 16px", borderBottom: `1px solid ${C.border2}`, fontFamily: "Space Mono,monospace", fontSize: 9, letterSpacing: 1, color: C.text3, textTransform: "uppercase" }}>
...
function AgentID({ toast }) {
  ...
  return (
    <div className="agentid-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      <Card delay={0}>
...
function Leaderboard({ toast }) {
  ...
  return (
    <div className="lb-grid" style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20 }}>
      <div>
...
["faq", "❓", "FAQ"],
  ...
  return (
    <div className="docs-layout" style={{ display: "grid", gridTemplateColumns: "220px 1fr", background: "rgba(13,13,26,0.9)", border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden", minHeight: 500 }}>
      <div className="docs-sidebar" style={{ borderRight: `1px solid ${C.border2}`, padding: "20px 0" }}>
...
export default function App() {
  ...
  const mintAgentID = async () => {
    if (!wallet) { toast("Connect wallet first"); return; }
    try {
      const { ethers } = await import("ethers");
      const provider = new ethers.BrowserProvider(window.ethereum);
      const signer = await provider.getSigner();
      const abi = ["function mint() external returns (uint256)"];
      const contract = new ethers.Contract("0x05587B4E0FD476dbFD59174EBC66e9B556512A8d", abi, signer);
      toast("Minting Agent ID on 0G Mainnet...");
      const tx = await contract.mint();
      await tx.wait();
      toast("Agent ID minted! Welcome to WRAITH");
    } catch (e) {
      toast("Mint failed: " + (e.reason || e.message || "error"));
    }
  };
    ...
    <div style={{ display: "flex", height: "100vh", position: "relative", background: C.bg }}>
      <style>{`
        @media (max-width: 768px) {
          .wraith-sidebar { display: none !important; }
          .wraith-main { width: 100vw !important; }
          .wraith-content { padding: 12px !important; }
        }
        ...
        @media (max-width: 768px) {
          ...
          .wb { display: flex !important; }
          .wc { padding: 12px 10px 80px !important; }
          .dash-grid { grid-template-columns: 1fr !important; }
          .stats-grid { grid-template-columns: 1fr 1fr !important; }
          .lb-grid { grid-template-columns: 1fr !important; }
          .agentid-grid { grid-template-columns: 1fr !important; }
          .docs-layout { grid-template-columns: 1fr !important; }
          .docs-sidebar { display: none !important; }
          .proof-table-head { display: none !important; }
          .proof-row-grid { grid-template-columns: 1fr auto !important; gap: 6px !important; }
        }
        @media (min-width: 769px) { .wb { display: none !important; } }
        .wb {
          position: fixed; bottom: 0; left: 0; right: 0; z-index: 200;
          background: rgba(7,7,15,0.98);
          border-top: 1px solid rgba(168,85,247,0.2);
          justify-content: space-around; align-items: center;
          padding: 8px 0 16px; backdrop-filter: blur(20px); display: none;
        }
        .wbn {
          display: flex; flex-direction: column; align-items: center; gap: 4px;
          cursor: pointer; border: none; background: transparent;
          color: rgba(240,236,255,0.4); font-size: 9px;
          font-family: Courier New,monospace; letter-spacing: 1px; min-width: 54px;
          padding: 4px;
        }
        .wbn.on { color: #c084fc; }
        .wbn span { font-size: 22px; line-height: 1.2; }
      `}</style>
      ...
      <aside className="wraith-sidebar" style={{ width: 220, flexShrink: 0, background: "rgba(13,13,26,0.95)", borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", backdropFilter: "blur(20px)", padding: "0 0 24px", position: "relative", zIndex: 10, minWidth: 0 }}>
      ...
      <div className="wraith-main" style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative", zIndex: 1 }}>
        <div style={{ padding: "0 28px", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${C.border2}`, background: "rgba(7,7,15,0.8)", backdropFilter: "blur(20px)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {wallet && <button onClick={mintAgentID} style={{ fontFamily: "Space Mono,monospace", fontSize: 10, color: "#fff", background: "linear-gradient(135deg,#34d399,#059669)", border: "none", padding: "6px 14px", borderRadius: 8, cursor: "pointer" }} onMouseEnter={e => e.target.style.opacity = "0.85"} onMouseLeave={e => e.target.style.opacity = "1"}>Mint Agent ID</button>}
          </div>
        </div>
        <div key={tab} className="wc" style={{ flex: 1, overflowY: "auto", padding: "16px 12px 32px", animation: "slideIn 0.35s ease", overflowX: "hidden" }}>
      ...
      <div className="wb">
        {[
          ...
          {id:"agentid",icon:"👁",label:"Agent"},
          {id:"leaderboard",icon:"🏆",label:"Ranks"},
          {id:"docs",icon:"📖",label:"Docs"}
        ].map(t=>(
          <button key={t.id} className={`wbn${tab===t.id?" on":""}`} onClick={()=>setTab(t.id)}>
